...
- Persistent apps use the Kopiur backup component and set `APP` to the exact PVC name being protected; new PVCs restore from `${APP}-nfs` by default via Kopiur `dataSourceRef`.
